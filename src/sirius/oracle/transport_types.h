// Double-precision spacetime types for the validation oracle
// (sirius::oracle): Vec4d, Mat4d, GeodesicStateD, HamiltonianStateD in the
// Boyer-Lindquist chart with index order (t, r, theta, phi). Off the render
// path. Ported from MTTP001A.h; numeric content bit-identical.
...
struct Vec4d {
    ...
    SIRIUS_HD double& operator[](int i) { return (&t)[i]; }

    SIRIUS_HD const double& operator[](int i) const { return (&t)[i]; }
    ...
    SIRIUS_HD Vec4d operator*(double s) const { return Vec4d(t * s, r * s, theta * s, phi * s); }
    ...
    // Negation
    SIRIUS_HD Vec4d operator-() const { return Vec4d(-t, -r, -theta, -phi); }

    // Euclidean norm squared (for debugging, not covariant)
    SIRIUS_HD double Norm2() const { return t * t + r * r + theta * theta + phi * phi; }

    // Zero check
    SIRIUS_HD bool IsZero() const { return t == 0 && r == 0 && theta == 0 && phi == 0; }
};

SIRIUS_HD inline Vec4d operator*(double s, const Vec4d& v) { return v * s; }
...
struct GeodesicStateD {
    ...
    SIRIUS_HD GeodesicStateD() : x(), k(), lambda(0), E(0), Lz(0), Q(0) {}
...
struct HamiltonianStateD {
    ...
    SIRIUS_HD HamiltonianStateD(const Vec4d& q_, const Vec4d& p_) : q(q_), p(p_), H(0) {}

    // Convert from GeodesicStateD
    SIRIUS_HD explicit HamiltonianStateD(const GeodesicStateD& geo) : q(geo.x), p(geo.k), H(0) {}
...
SIRIUS_HD inline Vec4d ToVec4d(float t, float r, float theta, float phi) {
    return Vec4d(static_cast<double>(t), static_cast<double>(r), static_cast<double>(theta),
...
struct Mat4d {
    ...
    // Zero matrix
    SIRIUS_HD static Mat4d Zero() { return Mat4d(); }
    ...
    SIRIUS_HD Mat4d operator+(const Mat4d& o) const {
        Mat4d result;
        for (int i = 0; i < 4; ++i)
            for (int j = 0; j < 4; ++j) result.m[i][j] = m[i][j] + o.m[i][j];
        return result;
    }
    ...
    SIRIUS_HD Mat4d operator*(double s) const {
        Mat4d result;
        for (int i = 0; i < 4; ++i)
            for (int j = 0; j < 4; ++j) result.m[i][j] = m[i][j] * s;
        return result;
    }
    ...
    // Trace
    SIRIUS_HD double Trace() const { return m[0][0] + m[1][1] + m[2][2] + m[3][3]; }
};
