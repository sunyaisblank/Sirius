// Geodesic Integration Accuracy Benchmarks
...
TEST_F(GeodesicBenchmarks, PhotonSphereRadius) {
    [[maybe_unused]] double r_photon = 3.0 * M;
...
TEST_F(GeodesicBenchmarks, WeakFieldLightDeflection) {
    ...
    for (double b : impact_params) {
        ...
        [[maybe_unused]] double arcsec = delta_phi * 180.0 / M_PI * 3600.0;
...
TEST_F(GeodesicBenchmarks, GeodesicConservedQuantities) {
    ...
    double r = 10.0 * M;
    [[maybe_unused]] double f = 1.0 - 2.0 * M / r;  // = 0.8
...
TEST_F(GeodesicBenchmarks, RadialFallProperTime) {
    ...
    double r0 = 10.0 * M;
    [[maybe_unused]] double rf = 3.0 * M;  // Just above photon sphere
