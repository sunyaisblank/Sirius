// Einstein-ring magnification oracle: ring radius, critical impact
// parameter, magnification scaling.
// Ported from TSGD002A.cpp; assertions and tolerances unchanged.
...
TEST(EinsteinRingTest, CriticalImpactParameter) {
    ...
    EXPECT_NEAR(b_critical, 5.196, 0.01) << "Critical impact parameter should be 3√3 M";
...
TEST(EinsteinRingTest, MagnificationDefinition) {
    ...
    EXPECT_NEAR(mu, 1.0, 1e-10) << "Flat space should have unit magnification";
}
...
TEST(EinsteinRingTest, DeflectionAngleFormula) {
    ...
    EXPECT_NEAR(alpha, 0.04, 1e-10) << "Weak-field deflection should be 4M/b";
}
...
TEST(EinsteinRingTest, MagnificationScaling) {
    ...
    EXPECT_NEAR(ratio1, deltas[0] / deltas[1], 0.2) << "Magnification should scale as 1/Δθ";
    EXPECT_NEAR(ratio2, deltas[1] / deltas[2], 0.2) << "Magnification should scale as 1/Δθ";
}
