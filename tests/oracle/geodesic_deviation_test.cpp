// Geodesic-deviation oracle: Riemann symmetries, flat-space deviation,
// ray-bundle ellipse semi-axes (fixed sum-of-squares discriminant form).
// Ported from TSCM007A.cpp; assertions and tolerances unchanged.
  ...
  protected:
    ...
    void computeSchwarzschildRiemann(double r, double M, double R_out[4][4][4][4]) {
        ...
        for (int i = 0; i < 4; ++i)
            for (int j = 0; j < 4; ++j)
                for (int k = 0; k < 4; ++k)
                    for (int l = 0; l < 4; ++l) R_out[i][j][k][l] = 0.0;
...
TEST_F(GeodesicDeviationTests, CircularBundleInitialization) {
    ...
    [[maybe_unused]] double xi1[4] = {0.0, 0.001, 0.0, 0.0};
    [[maybe_unused]] double xi2[4] = {0.0, 0.0, 0.001, 0.0};
