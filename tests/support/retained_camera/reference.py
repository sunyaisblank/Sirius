...
def prepare(observed_inputs=None):
    output = Path(__file__).resolve().parents[3] / "out" / "retained-camera-build"
    output.mkdir(parents=True, exist_ok=True)
    names, rows = fixtures()
    prefix = "moderate"
    if observed_inputs is not None:
        document = json.loads(Path(observed_inputs).read_text())
        if document["schema"] != "sirius-observed-camera-inputs-v1":
            raise ValueError("unknown observed input schema")
        names = [case["name"] for case in document["cases"]]
        rows = [list(struct.unpack("<32f", struct.pack("<32I", *[
            int(word, 16) for word in case["float32_bits"]]))) for case in document["cases"]]
        prefix = "original"
    references = []
    ...
    (output / (prefix + "-camera-inputs.bin")).write_bytes(
        b"".join(struct.pack("<32f", *row) for row in rows))
    (output / (prefix + "-camera-references.json")).write_text(
        json.dumps(references, indent=2) + "\n")
    return references


def freeze_cases():
    """Explicitly regenerate the finite witnesses consumed by the C++ tests."""
    import decimal
    source = Path(__file__).resolve().parent
    references = prepare() + prepare(source / "observed_inputs.json")
    cases = []
    with decimal.localcontext() as context:
        context.prec = 200
        for reference in references:
            values, gaps = [], []
            for low, high in zip(reference["reference100"]["scientific"],
                                 reference["reference180"]["scientific"]):
                high, low = decimal.Decimal(high), decimal.Decimal(low)
                values.append(format(high, ".85g"))
                gaps.append(str(2 * abs(high - low)))
            cases.append({"name": reference["name"], "input": reference["input"],
                          "scientific": values, "reference_gap": gaps})
    document = {
        "schema": "sirius-retained-camera-reference-cases-v1",
        "scope": "Independent 100/180-digit finite reference witnesses. The gap is an estimate, "
                 "not a certified interval. Generated by reference.py from its eight moderate "
                 "cases and observed_inputs.json.",
        "cases": cases,
    }
    (source / "reference_cases.json").write_text(json.dumps(document, indent=2) + "\n")


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--observed-inputs", type=Path)
    parser.add_argument("--freeze-cases", action="store_true")
    args = parser.parse_args()
    if args.freeze_cases:
        if args.observed_inputs:
            parser.error("--freeze-cases uses the checked-in observed inputs")
        freeze_cases()
    else:
        prepare(args.observed_inputs)
