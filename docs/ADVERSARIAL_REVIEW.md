...
| Registered correctness tests were omitted from the build-time `Mandatory` gate | CI and external full-estate runs included them, but an ordinary build could pass after running only a subset | Fixed: every enabled source GoogleTest is labelled `Mandatory`; the operational verifier compares all 734 parsed source identities with CTest's live JSON inventory, rejects missing or undeclared registrations and non-Mandatory labels, and permits only the explicitly conditional Vulkan session source as an all-or-nothing suite; category labels still support focused runs |
...
| P2900/P2996, P1–P6/E1–E4, and external-profile boundaries were prose-only at runtime | C++26 mode could be mistaken for native contracts/reflection and installed volumes carried no complete acceptance ledger | Fixed: compile-time language facts distinguish native features from checked-macro/explicit-schema substitutes; all ten P/E criteria, 23 dimensions, and 27 capability contracts are build-gated, installed, readiness-required, and exposed by `info capabilities` |
...
The same ledger now contains 27 explicit capability contracts. Each has one
...
- Install relocation, hostile-working-directory initialisation, and resource
  ...
  P/E criteria, 23 review dimensions, and 27 capability contracts. The final
