...
| Schwarzschild-de Sitter capture reused the asymptotically flat Kerr-Newman horizon, and pure de Sitter reported a zero-radius black-hole horizon | the public cosmological sector ignored both Kottler roots, admitted negative Lambda under de Sitter names, and allowed Nariai/super-Nariai scenes without a distinct black-hole exterior | Fixed: the spherical positive-Lambda Kottler authority solves and tests the smaller black-hole and larger cosmological roots of `1-2M/r-Lambda*r^2/3=0`; capture uses only the black-hole root, pure de Sitter never captures, named Schwarzschild-de Sitter requires `9 Lambda M^2 < 1`, and negative/rotating/charged cosmological requests decline. The renderer's horizon-penetrating ADM/Eulerian camera and directional sky boundary are stated explicitly rather than being mislabelled as static-observer cosmological transfer |
...
| Registered correctness tests were omitted from the build-time `Mandatory` gate | CI and external full-estate runs included them, but an ordinary build could pass after running only a subset | Fixed: every enabled source GoogleTest is labelled `Mandatory`; the operational verifier compares all 737 parsed source identities with CTest's live JSON inventory, rejects missing or undeclared registrations and non-Mandatory labels, and permits only the explicitly conditional Vulkan session source as an all-or-nothing suite; category labels still support focused runs |
...
| P2900/P2996, P1–P6/E1–E4, and external-profile boundaries were prose-only at runtime | C++26 mode could be mistaken for native contracts/reflection and installed volumes carried no complete acceptance ledger | Fixed: compile-time language facts distinguish native features from checked-macro/explicit-schema substitutes; all ten P/E criteria, 23 dimensions, and 28 capability contracts are build-gated, installed, readiness-required, and exposed by `info capabilities` |
...
| Spherical de Sitter/Kottler sector | `bounded`; exact positive-Lambda metric and both Kottler roots, with pure de Sitter horizonless for capture and Schwarzschild-de Sitter restricted below Nariai |
...
- Install relocation, hostile-working-directory initialisation, and resource
  ...
  P/E criteria, 23 review dimensions, and 28 capability contracts. The final
