...
| Finite-pupil camera launch | `core/camera_sampling.h`, `core/camera.h`, `backend/cpu/geodesic_tracer.cpp`, `kernels/gr_camera.slang`, and `kernels/trace.slang` | The exact-count camera sampler emits separate film and pupil dimensions; an analytic joint-moment oracle and structural substitutions reject the former two-dimensional diagonal. ThinLens samples carry an explicit camera-rest-frame pupil displacement as well as their focus-directed tangent. CPU and Slang move the actual launch event across the boosted orthonormal up/right axes and rebuild the frame at that event; a bounded local-pupil rule declines geometry too large for the tangent-plane construction. Host focus-plane geometry and a direct non-render Slang probe prevent a direction-only pinhole from returning. |
...
| Dispatch work and timing | `render/dispatch_governor.h` (`BandController`) | Bounds submitted work and adjusts subsequent submissions from observed cost. It cannot preempt a running kernel or guarantee its duration; the memory governor owns residency. |
...
and screen before transport; the analytic metric and its derivatives transform
by tensor pullback. Catalogue screens and azimuth-dependent matter samples map
back to the original convention. Horizon endpoints explicitly identify their
...
fixed-step oracle's symplectic property.
