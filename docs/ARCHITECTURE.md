...
| Display transfer encode | `core/srgb_transfer.h`, invoked once by each display writer | Strengthened. One host IEC 61966-2-1 curve and clipped round-to-nearest 8-bit quantiser serve explicitly normalised blackbody previews, explicit byte views, PNG, and PPM. PNG/PPM still own when encoding occurs, EXR remains linear, non-finite byte input declines, and repository governance rejects a second production breakpoint, detached consumer, or direct encoding of binned physical radiance. Independent output decode/round-trip tests remain the comparator. |
