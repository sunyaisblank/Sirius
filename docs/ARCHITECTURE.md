...
| Kerr circular orbits | `core/kerr_orbits.h` | Strengthened. Metric identity, Page-Thorne disk setup, invariant frequency transfer, and CPU tracer emitter motion share one exact signed-spin ISCO/angular-velocity authority. The Boyer-Lindquist oracle and Slang implementation remain independent comparators, with cross-consumer and device-parity gates preventing either reuse or drift. |
| Capture and causal-patch surfaces | `IMetric::InsideCaptureSurface`, analytic Kerr-Schild inverses, registry-owned exact Kottler black-hole/cosmological roots, `core/trace_boundary.h` | Carried and corrected for the spherical positive-Lambda sector. The smaller root owns capture; the larger root owns observer admission and the outer directional boundary. The CPU records every accepted segment's actual affine interval unconditionally and clips a finite crossing before advancing its central-ray consumers, so sky, Jacobi, polarisation, volume, and disk state cannot terminate at different events. |
...
| XYZ-D65 to linear sRGB | `core/xyz_srgb.h`, mirrored by `gr_disk.slang` and live parity-gated | Strengthened. Both host spectral facades consume one exact rational transform derived from the declared sRGB primaries and D65 white point. The device disk path owns one fp32 mirror, direct compute parity pins it independently, extended-gamut values remain linear instead of being hidden by this stage, and repository governance rejects rounded legacy coefficients, a second exact coefficient copy, or a detached consumer. |
...
| Display transfer encode | `core/srgb_transfer.h`, invoked once by each display writer | Strengthened. One host IEC 61966-2-1 curve and clipped round-to-nearest 8-bit quantiser serve spectral previews, explicit byte views, PNG, and PPM. PNG/PPM still own when encoding occurs, EXR remains linear, non-finite byte input declines, and repository governance rejects a second production breakpoint or detached consumer. Independent output decode/round-trip tests remain the comparator. |
...
The explicit host/device trace-parameter ABI has 65 occupied slots, numbered 0
through 64. Packing, bounds, and portability builds pin that literal boundary;
