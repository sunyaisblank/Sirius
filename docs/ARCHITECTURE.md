...
| Finite-pupil camera launch | `core/camera.h`, `backend/cpu/geodesic_tracer.cpp`, `kernels/gr_camera.slang`, and `kernels/trace.slang` | ThinLens samples carry an explicit camera-rest-frame pupil displacement as well as their focus-directed tangent. CPU and Slang move the actual launch event across the boosted orthonormal up/right axes and rebuild the frame at that event; a bounded local-pupil rule declines geometry too large for the tangent-plane construction. Host focus-plane geometry, a direct non-render Slang probe, and repository substitutions prevent a direction-only pinhole from returning. |
