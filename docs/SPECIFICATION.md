...
- P4, disk. The optically thick thin disk uses the zero-torque Page-Thorne flux and invariant circular-emitter frequency transfer exactly once. The bounded volumetric mode uses that radial temperature only under an explicit stationary, vertically isothermal Gaussian grey closure. The full circular-emitter transfer is the physical default; suppression substitutes the exact ZAMO frequency as a named nonphysical diagnostic. Gates cover independent Page-Thorne quadrature, its Newtonian cubic tail, Killing-field transfer, both live backends, and the measured diagnostic effect.
