...
- Integration boundary: pull requests and explicit integration dispatches configure the strict qualification topology on Linux, Windows, and macOS and compile every product and test executable, but execute only the named non-render governance and fail-closed authority controls. They must prove that no Mandatory-gate receipt exists before or after that run and may neither publish attestations nor claim qualification. Complete Mandatory/runtime execution and evidence publication occur only on protected-branch pushes and external-domain runbooks, so inexpensive integration evidence cannot be promoted into release evidence.
