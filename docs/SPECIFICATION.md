...
- P5, camera. Arbitrary camera four-velocity with aberration applied in the camera's local frame; pinhole and finite-aperture models; resolution to 5616 by 4096 within the memory governor. A finite-aperture sample starts at its metric-aware camera-rest-frame pupil event and points to the requested focus-plane point; direction jitter at one pinhole event is insufficient. The virtual pupil is bounded to $r_\mathrm{pupil}\le0.1\min(r_\mathrm{observer},d_\mathrm{focus})$, keeping the represented construction inside its local tangent-plane domain. Host focal convergence, direct Slang compute parity, and source negative controls gate that geometry before the physical full-frame requirement can be admitted.
