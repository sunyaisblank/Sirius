...
- P2, ray bundles. The renderer propagates the geodesic deviation equation alongside the central ray and derives each sample's footprint from the deviation solution. Both singular axes and the output-plane angle reach catalogue sampling in the exact deterministic celestial tangent basis in which that angle was measured; CPU and Slang share the least-aligned-axis convention. Gate: against analytic deviation solutions in Schwarzschild (radial and circular-orbit congruences) the propagated cross-section agrees to one part in 10^6 on the oracle; the live device basis agrees with the host authority; and a rotating-camera star-field sequence shows bounded per-star brightness variance where per-pixel point sampling measurably flickers.
